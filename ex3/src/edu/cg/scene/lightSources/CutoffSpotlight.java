...
public class CutoffSpotlight extends PointLight {
	...
	public boolean isOccludedBy(Surface surface, Ray rayToLight) {
		// Clac the angle of the incoming ray.
		Vec V = rayToLight.source().sub(this.position);
		Vec V_norm = V.normalize();
		double rayAngleCos = V_norm.dot(this.direction);
		double rayAngle = Math.toDegrees(Math.acos(rayAngleCos));

		if (rayAngle > this.cutoffAngle) {
			return false;
		}
	...
	public Vec intensity(Point hittingPoint, Ray rayToLight) {
		...
		return super.intensity(hittingPoint, rayToLight).mult(rayAngleCos);
	}
}