...
public class MenuWindow extends JFrame implements Logger {
	...
	public void removeObjectFromImage(boolean[][] srcMask) {
		...
		boolean[][] currentMask = duplicateMask();
		int currentWidth = workingImage.getWidth();
		RGBWeights rgbWeights = colorMixer.getRGBWeights();
		int maxCarvSize = workingImage.getWidth()/3 - 1;
		
		// Remove the marked pixels (masked pixels) from the image until no more masked pixel left.
		int trueCount = 0;
		do {
			// Find maximum true values in a row.
			trueCount = 0;
			for (int i=0; i < currentMask.length; i++){
				int rowBool = 0;
				for (boolean b : currentMask[i]) { 
					if (b) rowBool++;
				}
				if (rowBool > trueCount) trueCount = rowBool; 
			}
			// Make sure we bound the number of seams.
			currentWidth = currentWidth - Math.min(trueCount, maxCarvSize);
			SeamsCarver sc = new SeamsCarver(this, result, currentWidth, rgbWeights, currentMask);
			result = sc.resize();
			currentMask = sc.getMaskAfterSeamCarving();
			maxCarvSize = currentWidth/3 - 1;

		} while (trueCount != 0);

		// Increase the size of the carved image again.
		SeamsCarver sc = new SeamsCarver(this, result, workingImage.getWidth(), rgbWeights, currentMask);
		result = sc.resize();
